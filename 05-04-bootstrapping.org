...
#+property: header-args:rust :tangle src/axioms/bootstrap.rs

Arquivo: ~axioms/bootstrap.rs~.


# Acho  que algumas  dessas coisas  não  deviam aparecer  no livro.  É
# praticamente só digitação de AST.
...
#+begin_src majestic :eval no :tangle no
(def *ulps* 3)
#+end_src
